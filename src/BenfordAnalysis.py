...
def calculate_benford_distributions(election_results):
    ...
    for election_result in election_results:
        ...
        # add the candidate as the secondary key - initialize to empty leading digit array [0,0,...,0]
        if not benford_distributions.get(election_result.year).get(election_result.candidate):
            benford_distributions[election_result.year][election_result.candidate] = 9 * [0]
        ...
        # benford_distributions[election_result.year][election_result.candidate] = [100 * int(x) / int(election_result.totalvotes) for x in benford_distributions[election_result.year][election_result.candidate]]
